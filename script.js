...
// Declare variables
const studentName = "Latricha Seym";
...
function init() {
	...
	items = document.getElementsByClassName("studentGroup");
	for(var i = 0; i < items.length; i++){
		items[i].innerText = studentGroup;
	}
	
	items = document.getElementsByClassName("pageYear");
	for(var i = 0; i < items.length; i++){
		items[i].innerText = year;
	}
	
	items = document.getElementsByClassName("studentName");
	for(var i = 0; i < items.length; i++){
		items[i].innerText = studentName;
	}
	
    // Add navigation link to the navigation panel on the left of the page
    main.addContentItem("Inleiding", intro);
    main.addContentItem("Variabelen", variabeles);
    main.addContentItem("Object", object);
    main.addContentItem("Array", array);
    main.addContentItem("AJAX - tabel", ajax);
